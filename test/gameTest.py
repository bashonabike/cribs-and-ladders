...
import unittest
from cribbage.Player import BaseCribbageAgent
...
class TestAgent(BaseCribbageAgent):
    ...
    def pegging_move(self, hand, sequence, current_sum, current_pos):
        if self.pegging_func is not  None:
            return self.pegging_func(hand, sequence, current_sum, current_pos)
        else:
            BaseCribbageAgent.pegging_move(self, hand, sequence, current_pos)
